...
`VL_LIMIT_SYNC=1` 讓「自己沒有有效額度資料」的 session 可以借用其他 session 的讀數。每次 render 會把 `5h`／`7d` 的值寫進一個每台主機共用的 store（`limit-5h.d`、`limit-7d.d` 目錄）。session 只要自己有有效讀數就顯示自己的；自己沒有時 — payload 裡沒有 `rate_limits`，或視窗已經過期 — 就顯示任一 session 記錄過的最新視窗。store 裡若有更新的視窗也會勝出，所以還沒跟上換窗的 session 會跟著已經跟上的走。預設關閉。
...
同步刻意不會用別處較高的讀數蓋掉 session 自己的讀數。百分比在同一個視窗內是有可能合法下降的 — 官方重置額度、升級訂閱、任何伺服器端調整都會 — 而 payload 裡沒有任何欄位標記「這個值是什麼時候觀測到的」，所以一個過時的高值和一個當前的高值無法區分。若採用最大值，那個過時的值會被鎖到視窗結束為止，`5h` 最多五小時，`7d` 整整一週。
