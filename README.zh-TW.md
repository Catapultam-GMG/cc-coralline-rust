...
自己的讀數永遠贏自己的視窗。store 的角色是「自己完全沒有讀數時的來源」：payload 要等 session 收到 API 回應之後才會帶 rate limits，所以剛開、剛 resume、或閒置中的 session 原本會整個不顯示 gauge，即使帳號層級的視窗是已知的。store 裡只會有還沒關閉的視窗；而一筆讀數若所屬視窗關閉的時間已經超過一個視窗長度，就不再拿來代表當前視窗。
...
**別的 session 的讀數，永遠不會被當成你的顯示出來。** 同步不會用別處較高的讀數蓋掉你自己的，也不會在你完全沒有讀數時替你補一個 — 那種情況下就是不畫這個段落。兩個理由。百分比在同一個視窗內是有可能合法下降的（官方重置額度、升級訂閱、任何伺服器端調整），而 payload 裡沒有任何欄位標記「這個值是什麼時候觀測到的」，所以過時的高值和當前的高值無法區分。再者 store 沒有退場機制：一個視窗內曾經記錄過的最大值，會活得比回報它的那個 session 還久，`5h` 最多五小時，`7d` 整整一週。兩個視窗適用同一條規則，`5h` 只是因為換窗較快而讓問題比較不明顯。
